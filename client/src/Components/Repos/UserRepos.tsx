  ...
  return (
    <div className="space-y-1 overflow-y-auto h-screen scrollbar-thin scrollbar-[#0C0E15] md:h-[335px] lg:h-[400px]">
      {repos.map((item) => (
        <div
          key={item.id}
          className="bg-[#0C0E15] bg-opacity-60 border-l-4 border-l-violet-700 w-full h-28 p-3 rounded p-2 md:h-20 lg:h-24 md:border-l-2"
        >
          {/* LEFT SIDE */}
          ...
          <h5 className="text-white text-[10px] md:text-[8px] md:px-8 text-center mt-4 opacity-60">
            {item.description}
          </h5>
        </div>
      ))}
